...
def check(con_id_add, fio_add, temp_add, mem_add, tel_add, desc_add, dat_add, res_add, reg_dat):
    sql_cl = """SELECT*
                FROM Clients"""
    curs.execute(sql_cl)
    try:
        ...
        if cl_lst not in res:
            Cl_in.insert(con_id_add, fio_add, temp_add, mem_add, tel_add, desc_add, dat_add, res_add, reg_dat)
        else:
            pass
