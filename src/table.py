...
from src.variables import curs as cursor
from src.variables import conn as connect
from math import *
from tkinter import *


def create_sale_tab(curs, conn):
    count = 4
    sql_tb = """INSERT INTO Employees (FIO, Training_direction) VALUES (?,?)"""
    trainers = []# list of employees
    lesson = ['Силовой тренинг', 'Смешанные единоборства', 'Бокс', 'Кроссфит']
    # price = [13000, 27000, 25000, 19000]
    while count != 4:
        for z in trainers:
            for j in lesson:
                curs.execute(sql_tb, (z, j))
                conn.commit()
                count = count+1
